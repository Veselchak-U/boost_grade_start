...
import 'package:boost_grade_start/app/view/components/bgs/bgs_snake_bar.dart';
...
class _AsyncCallScreenState extends State<AsyncCallScreen> {
  ...
  Future<void> _requestPhotos(BuildContext context) async {
    _updateProgress(true);
    try {
      final result = await _dataProvider.getPhotos();
      _items = result ?? [];
    } catch (error) {
      BgsSnakeBar(context, error);
    } finally {
  ...
  Widget build(BuildContext context) {
    return BgsScaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () => _requestPhotos(context),
